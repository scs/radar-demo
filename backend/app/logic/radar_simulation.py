...
def current_send_step() -> int:
    ...
    send_step = int((current_steps[3] + radar_send_queue.qsize() + radar_receive_queue.qsize() + radar_result_queues[0].qsize() ) % STATIC_CONFIG.number_of_steps_in_period[3])
...
def synthetic_result(current_step: int, channel: int) -> NDArray[np.int32]:
    ...
    ypos = int((np.sin(phase + np.pi / 2) + 1) / 2 * 1023)
    xpos = 511 + int(
        (np.sin(phase)) * 511 / (STATIC_CONFIG.period_in_seconds[channel] / (STATIC_CONFIG.period_in_seconds[0] - 0.5))
    )
